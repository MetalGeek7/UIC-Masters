...
public class OntoProcessing {
	...
	/**
	 * Recursively adds parents of a Node including itself to the parentList
	 * @param startNode : Node for which parents are being added
	 * @param parentList : List of parents for the Node
	 * @return
	 */
	...
	/**
	 * Creates the initial blocks consisting of each concept as a block
	 * @param cns : List of concepts in the ontology
	 * @return an ArrayList of ArrayList i.e collection of blocks holding one or more concepts
	 */
	...
	/**
	 * Iteratively merges concepts into blocks till the required no. of blocks are created
	 * @param a
	 */
	...
	/**
	 * Merges two block(ArrayList of concepts) pointing at index locn and indexofCoup location i.e i and j 
	 * @param a: List of blocks
	 * @param i: Index location
	 * @param j: IndexofCoup location
	 * @return
	 */
	...
	/**
	 * Adds children of a node in the order of their occurance till a leaf node is encountered
	 * @param n: Node whose childen have to be added
	...
	public int calculateAnchors(ArrayList<CustomNode> sourceBlock,ArrayList<CustomNode> targetBlock,SimilarityMatrix m)
	{
		int anchors = 0;
	...
	public static double calculateProximity(ArrayList<CustomNode> arrayList,
		    ...
		    OntoProcessing processingObj = new  OntoProcessing();
		    int sharedAnchors = processingObj.calculateAnchors(arrayList,arrayList2,m);
		    int anchorsWithtarget=0,anchorsWithSource=0;
		    
		    for(int j=0;j<targetBlocks.size();j++)
		    	anchorsWithtarget += processingObj.calculateAnchors(arrayList,targetBlocks.get(j),m);
		    for(int i=0;i<targetBlocks.size();i++)
		         anchorsWithSource += processingObj.calculateAnchors(sourceBlocks.get(i),arrayList2,m);
