...
public class HungarianAlgorithm {
	...
	public final static String MAX_SUM_TYPE  = "max";
	public final static String MIN_SUM_TYPE  = "min";
	...
	{
		//rows must be <= cols
		//attention for this reason the assignment result may be switched
		if (array.length > array[0].length)
		{
			array = transpose(array);
		}
		double[][] cost = copyOf(array);	//Create the cost matrix
		if (sumType.equalsIgnoreCase(MAX_SUM_TYPE))	//Then array is weight array. Must change to cost.
		{
		...
		int[][] assignment = new int[array.length][2];	//Crea te the returned array.
	...
	{
		//Below enter MAX_SUM_TYPE or MIN_SUM_TYPE to find maximum sum or minimum sum assignment.
		String sumType = MAX_SUM_TYPE;		
